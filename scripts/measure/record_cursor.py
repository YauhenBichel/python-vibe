#!/usr/bin/env python3
"""Record the Cursor walkthrough: install MCP, then brief / ask / run.

  PYTHONPATH=src python scripts/measure/record_cursor.py

Needs asciinema, agg, and a running Ollama with llama3.1:8b (for ask).
Writes docs/media/cursor-demo.cast and docs/media/cursor-demo.gif.
...
CAST = MEDIA / "cursor-demo.cast"
GIF = MEDIA / "cursor-demo.gif"
WORK = Path("/tmp/cursor")
COLS, ROWS = 80, 24
PROMPT = b"demo $ "
...
def drive() -> None:
    """Sit in /tmp/cursor and type the four commands into bash."""
    os.chdir(WORK)
    env = os.environ.copy()
...
def _scrub_cast(path: Path) -> None:
    """Drop host paths from metadata. The session itself stays /tmp/cursor."""
    lines = path.read_text(encoding="utf-8").splitlines(keepends=True)
    if not lines:
        sys.exit("empty recording")
    head = json.loads(lines[0])
    head["command"] = "python3 scripts/measure/record_cursor.py --drive"
    head["env"] = {"SHELL": "/bin/bash", "TERM": "xterm-256color"}
    lines[0] = json.dumps(head, ensure_ascii=False) + "\n"
    text = "".join(lines)
    if "/Users/" in text or "DevBox/" in text:
        sys.exit("recording leaked a personal path; redo with /tmp/cursor only")
...
def record() -> None:
    ...
    rec = subprocess.run(
        [
            ...
            "2",
            "--command",
            "python3 scripts/measure/record_cursor.py --drive",
            str(CAST),
        ],
        cwd=str(ROOT),
        check=False,
