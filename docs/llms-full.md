...
- OpenCoder 8B and SWE-agent-LM 7B imported from Hub GGUF (`import_hf_ollama.py`). Write-tests 3/3 is the compiler bind. Idle one-word 7.42s. Helper off the 8B 36.69s. Already-listed helper 74.83s. Later an 8B client held `/api/chat`; empty helper not remasured. Default stays 8B. Do not pull 14B or 30B.
