...
- OpenCoder 8B and SWE-agent-LM 7B imported from Hub GGUF (`import_hf_ollama.py`). Write-tests 3/3 is the compiler bind. Listed SWE-agent missed a 60s `ok`. After `/api/ps` was empty, the same prompt finished in 6.8s. Default stays 8B. Do not pull 14B or 30B.
