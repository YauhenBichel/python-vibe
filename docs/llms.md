...
- [Which model to run]({{ '/investigations/which-model/' | absolute_url }}): Same-night daily: 8B 9/9, 7B coder 7/9. Extra tags timed out on clamp, including after a warm load (write-tests 3/3 is the compiler). 8B stays.
- [Model lanes]({{ '/investigations/model-lanes/' | absolute_url }}): Which local weight for which job. Default stays 8B.
- [Hub models]({{ '/investigations/hub-models/' | absolute_url }}): Hugging Face weights that fit 18 GB. OpenCoder 8B and SWE-agent-LM 7B are on disk (`import_hf_ollama.py`). Clamp timed out at 180s even after a warm load.
