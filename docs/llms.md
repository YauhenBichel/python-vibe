...
- [Which model to run]({{ '/investigations/which-model/' | absolute_url }}): Same-night daily: 8B 9/9, 7B coder 7/9. Bare clamp prompt 22–40s on four tags; daily clamp still timed out. 8B stays.
- [Model lanes]({{ '/investigations/model-lanes/' | absolute_url }}): Which local weight for which job. Default stays 8B.
- [Hub models]({{ '/investigations/hub-models/' | absolute_url }}): Hugging Face weights that fit 18 GB. SWE-agent-LM writes a bare clamp in 40s. Daily clamp still timed out. Do not switch.
