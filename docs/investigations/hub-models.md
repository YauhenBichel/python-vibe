...
| `swe-agent-lm:7b` | 4.7 GB | On disk. Empty-VRAM helper chat **43.45 s**. Earlier 300s empty miss was not stable. |
...
helper chat finished in 43.45 s (load 5.62 s). OpenCoder still
misses a one-word generate.
...
| [SWE-bench/SWE-agent-LM-7B](https://huggingface.co/SWE-bench/SWE-agent-LM-7B) | 7B, Apache-2.0 | `import_hf_ollama.py --name swe-agent-lm` | On disk as `swe-agent-lm:7b`. Empty-VRAM helper chat 43.45 s. Earlier 300s empty miss was not stable. |
