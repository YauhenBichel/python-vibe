...
| `swe-agent-lm:7b` | 4.7 GB | On disk. 8192-warmed daily clamp: first generate **180 s**. Stop SWE. |
...
generate at 180 s; after that `/api/ps` listed SWE. A later
one-word SWE load finished in 4.93 s, then the same daily job
with SWE listed timed out on the first generate at 180 s;
after that `/api/ps` was empty. A later Agent-body warm at
`num_ctx` 8192 finished in 29.76 s, then the same daily job
timed out on the first generate at 180 s; after that `/api/ps`
still listed SWE. Stop SWE. OpenCoder still misses a
one-word generate.
...
| [SWE-bench/SWE-agent-LM-7B](https://huggingface.co/SWE-bench/SWE-agent-LM-7B) | 7B, Apache-2.0 | `import_hf_ollama.py --name swe-agent-lm` | On disk as `swe-agent-lm:7b`. 8192-warmed daily clamp: first generate 180 s. Stop SWE. |
